...
exec' = do
  ...
  s1 `seq` s2 `seq` s3 `seq` (variants `using` parList (evalTuple2 r0 rseq)) `par` 
  	forkIO (parallel (map substituteAll chunks) >>= (\x -> atomically $ putTMVar store x))
  	-- do regex substitutions
  mapM_ putStrLn (results `using` parList rdeepseq)
  chunks' <- atomically $ takeTMVar store
...
parallel actions = do
    vars <- zipWithM (\ action cpu -> do
        ...
        return var) actions [0..]
    foldM (\ !x v -> liftM (+x) (atomically $ takeTMVar v)) 0 vars
