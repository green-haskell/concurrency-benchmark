...
processFile' :: IORef Int -> Int -> FilePath -> Int -> TChan Index -> TVar Int -> TVar (Set.Set Word) -> Buffer QueryIndex -> LogBuffer -> IO ()
processFile' capability taskId filePath maxFiles indexBuffer indexIdVar wordSetVar queryIndexBuffer logBuffer = do
    ...
    if ((numberOfFiles newIndex) < maxFiles)
        then atomically $ writeTChan indexBuffer newIndex
            ...
            _ <- forkOn cap $ createQueryIndex newIndex queryIndexBuffer logBuffer
            atomically $ do
...
processFile :: IORef Int -> Int -> Buffer FilePath -> Int -> TChan Index -> TVar Int -> TVar (Set.Set Word) -> Buffer QueryIndex -> LogBuffer -> SSem -> IO ()
processFile capability taskId fileBuffer maxFiles indexBuffer indexIdVar wordSetVar queryIndexBuffer logBuffer finishProcessing = do
    next <- atomically $ readBuffer fileBuffer
    case next of
        Nothing -> atomically $ Sem.signal finishProcessing
        Just filePath -> do
            processFile' capability taskId filePath maxFiles indexBuffer indexIdVar wordSetVar queryIndexBuffer logBuffer
            processFile capability taskId fileBuffer maxFiles indexBuffer indexIdVar wordSetVar queryIndexBuffer logBuffer finishProcessing


processFiles :: IORef Int -> Int -> Int -> Int -> Buffer FilePath -> Buffer QueryIndex -> LogBuffer -> IO ()
processFiles capability initialSubIndices maxFiles nWorkers fileBuffer queryIndexBuffer logBuffer = do
    ...
    cap <- atomicModifyIORef' capability (\x -> (x + nWorkers, x))
    forM_ [1..nWorkers] $ \taskId ->
        let c = taskId - 1 + cap in
        forkOn c $ processFile capability taskId fileBuffer maxFiles indexBuffer indexIdVar wordSetVar queryIndexBuffer logBuffer finishProcessing

    cap <- atomicModifyIORef' capability (\x -> (x + 1, x))
    _ <- forkOn cap $ waiter finishProcessing indexBuffer queryIndexBuffer logBuffer
    return ()
...
processSearch :: IORef Int -> [String] -> Buffer QueryIndex -> LogBuffer -> IO SSem
processSearch capability rawQueries queryIndexBuffer logBuffer = do
    ...
    slaveBuffers <- forM rawQueries $ \rawQuery -> do
        ...
        cap <- atomicModifyIORef' capability (\x -> (x + 1, x))
        _ <- forkOn cap $ Engine.search (Query.parse rawQuery) qBuffer (0, []) finishSearch logBuffer
        return qBuffer

    cap <- atomicModifyIORef' capability (\x -> (x + 1, x))
    _ <- forkOn cap $ Engine.searchMultiplexer queryIndexBuffer slaveBuffers
