...
exec' w = do
    ...
    q <- newChan
    replies <- replicateM w newEmptyTMVarIO
    mapM_ (writeChan q) $ zip coords replies
    forM_ [0..numCapabilities-1] $ \cpu -> forkOn cpu $ worker q w m n

    putStrLn ("P4\n"++show w++" "++show w)
    mapM_ ((atomically . takeTMVar) >=> \b -> hPutBuf stdout b n) replies
