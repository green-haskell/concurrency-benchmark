...
run :: Int -> IORef Int -> Int -> [Color] -> IO (IO ())
run n cap _ cs = do
    fs    <- replicateM (length cs) newEmptyTMVarIO
    mpv   <- newTMVarIO (Nobody n)
    withArrayLen cs $ \ n cols -> do
        c <- atomicModifyIORef' cap (\x -> (x + 1, x))
        zipWithM_ ((forkOn c .) . arrive mpv) fs (take n (iterate (`advancePtr` 1) cols))

        return $ do
          putStrLn . map toLower . unwords . ([]:) . map show $ cs
          ns    <- mapM (\x -> atomically $ takeTMVar x) fs
...
exec' n = do
    ...
    actions <- zipWithM (run n capability) [0..] [[B..Y],[B,R,Y,R,Y,B,R,Y,R,B]]
    sequence_ actions
