...
processFile' taskId filePath maxFiles indexBuffer indexIdVar wordSetVar queryIndexBuffer logBuffer = do
    ...
    if ((numberOfFiles newIndex) < maxFiles)
        then atomically $ writeTChan indexBuffer newIndex
            ...
            _ <- forkOS $ createQueryIndex newIndex queryIndexBuffer logBuffer
            atomically $ do
...
processFiles initialSubIndices maxFiles nWorkers fileBuffer queryIndexBuffer logBuffer = do
    ...
    forM_ [1..nWorkers] $ \taskId ->
        forkOS $ processFile taskId fileBuffer maxFiles indexBuffer indexIdVar wordSetVar queryIndexBuffer logBuffer finishProcessing

    _ <- forkOS $ waiter finishProcessing indexBuffer queryIndexBuffer logBuffer
    return ()
...
processSearch rawQueries queryIndexBuffer logBuffer = do
    ...
    slaveBuffers <- forM rawQueries $ \rawQuery -> do
        ...
        _ <- forkOS $ Engine.search (Query.parse rawQuery) qBuffer (0, []) finishSearch logBuffer
        return qBuffer

    _ <- forkOS $ Engine.searchMultiplexer queryIndexBuffer slaveBuffers
