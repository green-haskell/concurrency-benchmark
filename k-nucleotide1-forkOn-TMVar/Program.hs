...
import Utils(redirectStdIOAndRun)
...
exec input = redirectStdIOAndRun exec' input
...
tcalculate input size = do
    ...
    vars <- zipWithM (\action cpu -> do
                    var <- newEmptyTMVarIO
                    forkOn cpu $ do
                        answer <- action
                        atomically $ putTMVar var answer
                    return var) actions l
    result <- newTable :: IO HM
    results <- mapM (atomically . takeTMVar) vars
    mapM_ (\ht -> Program.foldM (\lst (k,v) -> do 
                            res <- Program.lookup lst k
                            case res of
                                Nothing -> do
                                    r1 <- peek v
                                    r2 <- malloc
                                    poke r2 r1
                                    Program.insert lst k r2
                                Just v1 -> do
                                    r1 <- peek v1
                                    r2 <- peek v
                                    poke v1 (r1+r2)
